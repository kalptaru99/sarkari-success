  ...
  return (
    <main style={{ minHeight: '100vh', backgroundColor: '#f4f6f9', fontFamily: 'Arial, sans-serif' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #1e3a8a 0%, #1e40af 50%, #dc2626 100%)', padding: '50px 20px', textAlign: 'center' }}>
        ...
        <p style={{ color: '#fca5a5', fontSize: '15px', margin: '0 0 32px 0', fontWeight: '600' }}>
          🎯 Ask SarkariGPT anything — get instant answers in Hindi, Tamil, Telugu & 10 more languages
        </p>

        <div style={{ maxWidth: '650px', margin: '0 auto 24px auto', backgroundColor: 'white', borderRadius: '50px', padding: '6px 6px 6px 20px', display: 'flex', alignItems: 'center', gap: '10px', boxShadow: '0 8px 32px rgba(0,0,0,0.2)' }}>
          <span style={{ fontSize: '20px' }}>🤖</span>
          <input
            type="text"
            placeholder="SSC CGL 2026 ke baare mein poochein ya koi bhi sawaal..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && window.location.href === '/sarkarigpt'}
            style={{ flex: 1, border: 'none', outline: 'none', fontSize: '15px', color: '#1a1a1a', backgroundColor: 'transparent' }}
          />
          <a href={"/sarkarigpt" + (searchQuery ? "?q=" + encodeURIComponent(searchQuery) : "")}
            style={{ backgroundColor: '#dc2626', color: 'white', padding: '12px 24px', borderRadius: '40px', textDecoration: 'none', fontSize: '15px', fontWeight: 'bold', flexShrink: 0 }}>
            Ask AI →
          </a>
        </div>
